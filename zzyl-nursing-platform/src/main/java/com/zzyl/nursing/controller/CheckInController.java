...
public class CheckInController extends BaseController
{
     ...
     */
    @ApiOperation("申请入住")
    @PreAuthorize("@ss.hasPermi('nursing:checkIn:apply')")
    @Log(title = "入住申请", businessType = BusinessType.INSERT)
    @PostMapping("/apply")
    public AjaxResult apply(@RequestBody @ApiParam("申请入住的对象") CheckInApplyDto dto)
    {
     ...
     */
    @ApiOperation("获取入住详情")
    @PreAuthorize("@ss.hasPermi('nursing:checkIn:detail')")
    @GetMapping("/detail/{id}")
    public R<CheckInDetailVo> getDetail(@PathVariable("id") @ApiParam("入住ID") Long id)
    {
        return R.ok(checkInService.getDetail(id));
    }

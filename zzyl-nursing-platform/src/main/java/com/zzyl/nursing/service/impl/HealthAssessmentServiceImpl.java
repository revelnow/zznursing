...
public class HealthAssessmentServiceImpl extends ServiceImpl<HealthAssessmentMapper, HealthAssessment> implements IHealthAssessmentService
{
    ...
    public Long insertHealthAssessment(HealthAssessment healthAssessment)
    {
        ...
        //2.调用qianwen模型进行分析，得到结果
        String aiResult = aiModelInvoker.bailianInvoker( prompt);

        //3.将结果转换为HealthAssessment对象进行存储
        // 这里的转换过程比较复杂，需要根据AI模型返回的JSON格式进行解析，并将解析后的数据填充到HealthAssessment对象中。
        HealthReportVo healthReportVo = JSON.parseObject(aiResult, HealthReportVo.class);
    ...
    private String getPrompt(String idCard) {
        // 1.从Redis中获取当前身份证号对应的体检报告
        String report = (String) redisTemplate.opsForHash().get("healthReport", idCard);
       ...
       throw new RuntimeException("未找到对应的体检报告");
       }
